...
const SignupForm = () => {
  ...
  const onFormSubmit = async (e) => {
    ...
    if (!formData.policy) {
      return setError([{ message: "You must accept the privacy policy." }]);
    }
    ...
    try {
      ...
      if (!res.ok) {
        return setError([{ message: data.message || "Registration failed" }]);
      }

      Cookie.set("access_token", encode(data.token));
      router.push("/");
    } catch (err) {
